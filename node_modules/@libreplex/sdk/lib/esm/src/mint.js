import { Keypair, PublicKey, SYSVAR_SLOT_HASHES_PUBKEY, SystemProgram, Transaction, TransactionInstruction, } from '@solana/web3.js';
...
import { TOKEN_2022_PROGRAM_ID, createInitializeMint2Instruction, getAssociatedTokenAddressSync, createAssociatedTokenAccountInstruction, createMintToInstruction, } from '@solana/spl-token';
...
export async function mintFromCreatorControllerState(input) {
...
export async function mintFromCreatorController(input) {
...
export async function mintSingle(input) {
    const { provider, metadataProgram = await loadMetadataProgram(provider), nftProgram = await loadNftProgram(provider), mintToCollection, receiver = provider.publicKey, mintKp = Keypair.generate(), transferHook, updateAuthority = provider.publicKey, mintData, } = input;
    ...
    const mintCtx = await setupLibreplexReadyMint(connection, me, me, receiver, me, 0, mintKp, metadata, transferHook);
    let anchorAssetUrl;
    ...
    if (mintToCollection) {
        const ix = await metadataProgram.methods
            ...
            delegatedCollectionWidePermissions: mintToCollection.groupDelegate
                ? getCollectionWideUserPermissionsAddress(mintToCollection.collection, me)
                : null,
            systemProgram: SystemProgram.programId,
            payer: me,
            metadata,
...
const initializeMetadataPointerInstructionData = struct([
    u8('instruction'),
    u8('metadataPointerInitIx'),
    publicKey('authority'),
    publicKey('metadataAddress'),
]);
const initializeTransferHookInitInstructionData = struct([
    u8('instruction'),
    u8('transferHookInstruction'),
    publicKey('authority'),
    publicKey('transferHookProgramId'),
]);
export async function setupLibreplexReadyMint(connection, payer, receiver, mintAuthority, freezeAuthority, decimals, mintKeypair = Keypair.generate(), metadata, transferHook, programId = TOKEN_2022_PROGRAM_ID) {
